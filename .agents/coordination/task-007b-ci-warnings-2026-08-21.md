...
- Harness retro TODO: AH-031 implemented with a canonical Compose project name and multi-worktree regression coverage (renumbered during main integration because TASK-017 already owns AH-030).
...
| AH-031 | Make shared Compose service discovery independent of linked-worktree paths | `compose.yml`, `scripts/agent-harness/test-task-worktrees.sh`, `TODO-AGENT-HARNESS.md` | `npm run agents:validate` and `npm run task:preflight` passed from TASK-007B |
